...
*********************************************************************************************/
function bestthemeframework_archive_nag(){
    global $pagenow;
    if ( $pagenow == 'themes.php' ) {
         echo '<div class="updated"><p>';
		 _e('Portfolio archive pages will only display in WordPress 3.1 or above.  Please upgrade.', 'bestthemeframework');
		 echo '</p></div>';
    }
}

if ( get_bloginfo('version') < 3.1 ) {
	add_action('admin_notices', 'bestthemeframework_archive_nag');
}
...
*********************************************************************************************/
add_filter( 'wp_get_attachment_link', 'bestthemeframework_prettyphoto');

function bestthemeframework_prettyphoto ($content) {
...
function wpthemess_content_nav() {
	...
	if (  $wp_query->max_num_pages > 1 ) :
		if (function_exists('wp_pagenavi') ) {
        	...
        	<nav id="nav-below">
			<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'bestthemeframework' ); ?></h1>
			<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'bestthemeframework' ) ); ?></div>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'bestthemeframework' ) ); ?></div>
			</nav><!-- #nav-below -->
    	<?php }
	endif;
}
...
*********************************************************************************************/
function bestthemeframework_comments($comment, $args, $depth) {
  ...
  <li <?php comment_class(); ?>>
    <article id="comment-<?php comment_ID(); ?>">
      ...
      <em style="display:block;margin-left:40px"><?php _e('Your comment is awaiting moderation.','bestthemeframework') ?></em>
      <?php endif; ?>
